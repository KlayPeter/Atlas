...
export function htmlEnhancePrompt(request: HtmlEnhanceRequest) {
  ...
  return [
    ...
    readableMode
      ? [
          '生成“易读版正文”，不只是给原文添加摘要。',
          '先识别原文已有的主题边界、定义、步骤、并列项和对比关系；仅在这些关系明确存在时，用小标题、列表或表格呈现它们。',
          '每段只推进一个主题。拆分长句和密集段落，优先使用具体主语和明确动词；删去不承载信息的套话，但不要把作者的语气改成宣传文案。',
          '用通俗语言解释术语，并在首次出现时保留原术语；可以补充原文已支持的因果、转折或先后关系。',
          '必须保留所有重要事实、数字、人名、日期、条件、结论、引用、URL、代码和不确定性；不得增加原文没有的事实、例子、动机、引用或确定性。',
          '保持事实/观点/假设/引语之间的区别。不要改写代码、公式、引语、链接目标或专业术语的准确含义。',
          '若原文已足够清楚，优先做最小的结构整理，不要为了“更像 AI”而过度改写。',
          '完成前按读者视角自检：每段主题是否清楚、相邻段落关系是否可理解、读者能否定位主张及其条件；再核对数字、实体、否定、条件、引用与结论均未丢失。',
